// COMPONENT: PARTNER VIEW (SPARE PARTS)
// ============================================================================
// MASTER PARTNER SETTLEMENT ENGINE v2.5 - SPARE PARTS DIVISION
// ============================================================================
/**
 * Provides high-precision financial tracking for the Spare Parts partner.
 * 
 * CORE LOGIC FEATURES:
...
window.PartnerSpareParts = function() {
  ...
  // Fixed department identifier for the Spare Parts sector
  const deptID = "spare_parts";
  ...
  // Step B: Filter all previous payout records for Spare Parts
  const sparePartsPaymentHistory = partnerPayments.filter(function(p) {
  ...
  const auditedTransactions = txRange.map(function(tx) {
    ...
    const wasThisTxPaid = sparePartsPaymentHistory.some(function(payout) {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6 font-sans pb-32 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 no-print">
        <div>
           <h1 className="text-2xl font-black text-white uppercase italic leading-none">{t("spareParts")}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* Card 3: Spare Parts share with Pay Button or Paid Label */}
        <div className={"p-5 rounded-2xl border shadow-2xl transition-all duration-500 relative overflow-hidden " + (isRangeFullyPaid ? "bg-blue-900/20 border-blue-500/30" : "bg-orange-500/10 border-orange-500/30 shadow-orange-500/5")}>
           <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Spare Parts Balance</div>
      ...
      {/* SECTION: SPARE PARTS PAYOUT HISTORY */}
      <div className="rounded-2xl border bg-gray-800 border-gray-700 overflow-hidden shadow-xl">
        ...
        <div className="divide-y divide-gray-700/50">
          {sparePartsPaymentHistory.length === 0 ? <div className="p-10 text-center text-gray-600 text-[10px] font-bold uppercase tracking-widest opacity-30">No recorded payments</div> :
            sparePartsPaymentHistory.map(function(payment) {
              return (
                ...
                <div key={tx.id} className={"flex items-center px-6 py-5 gap-4 transition-colors " + (tx.isCleared ? "opacity-30 bg-black/10" : "hover:bg-gray-700/40")}>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                       <div className={"text-sm font-black uppercase tracking-tight " + (tx.isCleared ? "text-gray-500 line-through" : "text-gray-200")}>{tx.description || "Spare Parts Service"}</div>
