// COMPONENT: PARTNER VIEW (MECHANIC)
// ============================================================================
// MASTER PARTNER SETTLEMENT ENGINE v2.5 - MECHANIC DIVISION
// ============================================================================
// 
// This module provides the high-precision financial view for the Mechanic.
// 
// CORE LOGIC FEATURES:
...
window.PartnerMechanic = function() {
  
  // -- EXTERNAL DATA HOOKS --
  const { t } = window.useLang();
  
  // Accessing master cloud data and settlement functions
  const { 
    transactions, 
    partnerPayments, 
    addPartnerPayment 
  ...
  // Fixed department identifier for the Mechanic sector
  const deptID = "mechanic";
  ...
  /**
   * handleDeletePayment
   * logic: Permanently removes a payout record from the cloud.
   */
  ...
  /**
   * handleUpdatePayment
   * logic: Saves changes made to a past payment record.
   */
  ...
  // Step B: Filter all previous payout records for the Mechanic
  const mechanicPaymentHistory = partnerPayments.filter(function(p) {
  ...
  const auditedTransactions = txRange.map(function(tx) {
    ...
    const wasThisTxPaid = mechanicPaymentHistory.some(function(payout) {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6 font-sans pb-32 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 no-print">
        <div>
           <h1 className="text-2xl font-black text-white uppercase italic leading-none">{t("mechanic")}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-800 p-5 rounded-2xl border border-gray-700 shadow-xl">
           <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Total Range Sales</div>
           <div className="text-2xl font-black text-white tabular-nums tracking-tighter">{window.formatAFN(totalRevenueInRange)}</div>
        </div>

        <div className="bg-gray-800 p-5 rounded-2xl border border-gray-700 shadow-xl opacity-90">
           ...
           <div className="text-[9px] text-green-500 font-black uppercase mt-2">Status: Already Paid</div>
        </div>

        <div className={"p-5 rounded-2xl border shadow-2xl transition-all duration-500 " + (isRangeFullyPaid ? "bg-blue-900/20 border-blue-500/30" : "bg-orange-500/10 border-orange-500/30 shadow-orange-500/5")}>
           <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Mechanic Balance</div>
      ...
      {/* SECTION: MECHANIC PAYOUT HISTORY */}
      <div className="rounded-2xl border bg-gray-800 border-gray-700 overflow-hidden shadow-xl">
        ...
        <div className="divide-y divide-gray-700/50">
          {mechanicPaymentHistory.length === 0 ? <div className="p-10 text-center text-gray-600 text-[10px] font-bold uppercase tracking-widest opacity-30">No recorded payments</div> :
            mechanicPaymentHistory.map(function(payment) {
              return (
      ...
      <div className="rounded-2xl border bg-gray-800 border-gray-700 overflow-hidden shadow-2xl">
        ...
        <div className="divide-y divide-gray-700/50">
          {auditedTransactions.length === 0 ? <div className="p-16 text-center text-gray-700 italic text-[10px] font-bold uppercase tracking-widest opacity-20">No services performed in this range</div> :
                ...
                <div key={tx.id} className={"flex items-center px-6 py-5 gap-4 transition-all " + (tx.isCleared ? "opacity-30 bg-black/10" : "hover:bg-gray-700/40")}>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                       <div className={"text-sm font-black uppercase tracking-tight " + (tx.isCleared ? "text-gray-500 line-through" : "text-gray-200")}>{tx.description || "Mechanic Service"}</div>
